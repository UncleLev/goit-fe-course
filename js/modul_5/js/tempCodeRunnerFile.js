 ...
 */
const getAllNames = arr => arr.map(ell => ell.name);
 ...
 */
const getUsersByEyeColor = (arr, color) => arr.filter(ell => ell.eyeColor == color);
 ...
 */
const getUsersByGender = (arr, gender) => arr.filter(ell => ell.gender == gender).map(ell => ell.name);
 ...
 */
const getInactiveUsers = arr => arr.filter(ell => ell.isActive != true).map(ell => ell.name);
 ...
 */
const getUserByEmail = (arr, email) => arr.find(ell => ell.email == email).name;
 ...
 */
const getUsersWithAge = (arr, min, max) => arr.filter(ell => ell.age > min && ell.age < max);
 ...
 */
const getTotalBalance = arr => arr.reduce((acc, ell) => acc + ell.balance, 0);
 ...
 */
const getUsersByFriend = (arr, name) => arr.filter(ell => ell.friends.includes(name)).map(ell => ell.name);
...
const getAllUniqueSkills = arr => {
  ...
  arr.map(el => {
      el.skills.map(el => {
        if(!allSkils.includes(el)){
          allSkils.push(el)
        }
      });
    
  })
  return allSkils.sort();
};
