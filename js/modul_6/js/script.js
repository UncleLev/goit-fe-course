...
class Hamburger {
   ...
   */
  calculatePrice() {
    let totalStufPrice = Hamburger.STUFFINGS[this._stuffing].price;  
    let totalPrice = Hamburger.SIZES[this._size].price;
    let totalTopPrice = this._toppings.reduce((acc, el) => Hamburger.TOPPINGS[el].price + acc, 0);
    return totalPrice + totalTopPrice;
  }
   ...
   */
  calculateCalories() {
    let totalCal = Hamburger.SIZES[this._size].calories;
    let totalTopCal = this._toppings.reduce((acc, el) => Hamburger.TOPPINGS[el].calories + acc, 0);
    let totalStufCal = Hamburger.STUFFINGS[this._stuffing].calories; 
    return totalCal + totalTopCal + totalStufCal;
  }
}
